// @generated by protobuf-ts 1.0.13 with parameters long_type_string,optimize_code_size,generate_dependencies,optimize_code_size
// @generated from protobuf file "profile/v1/profile.proto" (package "protocol.profile.v1", syntax proto3)
// tslint:disable
import { RpcTransport } from "@protobuf-ts/runtime-rpc";
import { MethodInfo } from "@protobuf-ts/runtime-rpc";
import { SetAppDataResponse } from "./types";
import { SetAppDataRequest } from "./types";
import { GetAppDataResponse } from "./types";
import { GetAppDataRequest } from "./types";
import { UpdateProfileResponse } from "./types";
import { UpdateProfileRequest } from "./types";
import { stackIntercept } from "@protobuf-ts/runtime-rpc";
import { GetProfileResponse } from "./types";
import { GetProfileRequest } from "./types";
import { UnaryCall } from "@protobuf-ts/runtime-rpc";
import { RpcOptions } from "@protobuf-ts/runtime-rpc";
/**
 * Harmony's Profile service manages the profiles of the users.
 *
 * @generated from protobuf service protocol.profile.v1.ProfileService
 */
export interface IProfileServiceClient {
    /**
     * Gets a user's profile.
     *
     * @generated from protobuf rpc: GetProfile(protocol.profile.v1.GetProfileRequest) returns (protocol.profile.v1.GetProfileResponse);
     */
    getProfile(input: GetProfileRequest, options?: RpcOptions): UnaryCall<GetProfileRequest, GetProfileResponse>;
    /**
     * Updates the user's profile.
     *
     * @generated from protobuf rpc: UpdateProfile(protocol.profile.v1.UpdateProfileRequest) returns (protocol.profile.v1.UpdateProfileResponse);
     */
    updateProfile(input: UpdateProfileRequest, options?: RpcOptions): UnaryCall<UpdateProfileRequest, UpdateProfileResponse>;
    /**
     * Gets app data for a user (this can be used to store user preferences which
     * is synchronized across devices).
     *
     * @generated from protobuf rpc: GetAppData(protocol.profile.v1.GetAppDataRequest) returns (protocol.profile.v1.GetAppDataResponse);
     */
    getAppData(input: GetAppDataRequest, options?: RpcOptions): UnaryCall<GetAppDataRequest, GetAppDataResponse>;
    /**
     * Sets the app data for a user.
     *
     * @generated from protobuf rpc: SetAppData(protocol.profile.v1.SetAppDataRequest) returns (protocol.profile.v1.SetAppDataResponse);
     */
    setAppData(input: SetAppDataRequest, options?: RpcOptions): UnaryCall<SetAppDataRequest, SetAppDataResponse>;
}
/**
 * Harmony's Profile service manages the profiles of the users.
 *
 * @generated from protobuf service protocol.profile.v1.ProfileService
 */
export class ProfileServiceClient implements IProfileServiceClient {
    readonly typeName = "protocol.profile.v1.ProfileService";
    readonly methods: MethodInfo[] = [
        { service: this, name: "GetProfile", localName: "getProfile", I: GetProfileRequest, O: GetProfileResponse, options: { "protocol.harmonytypes.v1.metadata": { requiresAuthentication: true } } },
        { service: this, name: "UpdateProfile", localName: "updateProfile", I: UpdateProfileRequest, O: UpdateProfileResponse },
        { service: this, name: "GetAppData", localName: "getAppData", I: GetAppDataRequest, O: GetAppDataResponse, options: { "protocol.harmonytypes.v1.metadata": { requiresAuthentication: true } } },
        { service: this, name: "SetAppData", localName: "setAppData", I: SetAppDataRequest, O: SetAppDataResponse }
    ];
    constructor(private readonly _transport: RpcTransport) {
    }
    getProfile(input: GetProfileRequest, options?: RpcOptions): UnaryCall<GetProfileRequest, GetProfileResponse> {
        const method = this.methods[0], opt = this._transport.mergeOptions(options), i = method.I.create(input);
        return stackIntercept<GetProfileRequest, GetProfileResponse>("unary", this._transport, method, opt, i);
    }
    updateProfile(input: UpdateProfileRequest, options?: RpcOptions): UnaryCall<UpdateProfileRequest, UpdateProfileResponse> {
        const method = this.methods[1], opt = this._transport.mergeOptions(options), i = method.I.create(input);
        return stackIntercept<UpdateProfileRequest, UpdateProfileResponse>("unary", this._transport, method, opt, i);
    }
    getAppData(input: GetAppDataRequest, options?: RpcOptions): UnaryCall<GetAppDataRequest, GetAppDataResponse> {
        const method = this.methods[2], opt = this._transport.mergeOptions(options), i = method.I.create(input);
        return stackIntercept<GetAppDataRequest, GetAppDataResponse>("unary", this._transport, method, opt, i);
    }
    setAppData(input: SetAppDataRequest, options?: RpcOptions): UnaryCall<SetAppDataRequest, SetAppDataResponse> {
        const method = this.methods[3], opt = this._transport.mergeOptions(options), i = method.I.create(input);
        return stackIntercept<SetAppDataRequest, SetAppDataResponse>("unary", this._transport, method, opt, i);
    }
}
